...
For your server to be accessible from the internet, you must configure **Port Forwarding** on your home router. This tells the router which internal device (your server) should handle incoming traffic on specific ports.

### Required Port Forwards

| Service | Internal Port | External Port (Recommended) | Description |
| :--- | :--- | :--- | :--- |
| **HTTP** | 80 | 80 | Required for initial ACME challenges and redirects. |
| **HTTPS** | 443 | 443 | Main encrypted web traffic (proxied by Cloudflare). |
| **SSH** | 22 | **2222** | Remote CLI access. Using a non-standard external port reduces bot noise. |

### Security Tip: Non-Standard SSH Port
By mapping an unusual external port (like `2222`) to the internal port `22` of your server, you avoid 99% of automated "script kiddie" scans that only target the default port 22. 

To connect using a non-standard port:
```bash
