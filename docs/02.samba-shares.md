...
The complete list of available options for the `services.samba` module can be found in the [official NixOS Service Search](https://search.nixos.org/options?channel=25.11&query=services.samba).

## Full Configuration Template

You can use this template in `modules/services/samba.nix` to configure the service:
...
Use these commands to monitor the Samba service and discovery protocols:

### View Service Logs
- **Samba Daemon:**
  ```bash
  journalctl -u samba-smbd.service -f
  ```
- **Discovery (Avahi/WSDD):**
  ```bash
  journalctl -u avahi-daemon.service -f
  journalctl -u samba-wsdd.service -f
  ```
...
### Active Connections
To see who is currently connected to the shares:
```bash
smbstatus
```
