...
The complete list of available options for the `services.syncthing` module can be found in the [official NixOS Service Search](https://search.nixos.org/options?channel=25.11&query=services.syncthing) when choosing the right package channel (currently 25.11).
...
{ config, pkgs, ... }: {
  services.syncthing = {
    ...
    settings = {
      ...
      devices = {
        "DEVICE-NAME" = { id = "XXXXXXX-XXXXXXX-XXXXXXX"; };
      };
...
When the Web GUI is not accessible or you are working directly in the terminal, use these commands to monitor the service:

### View Service Logs
Since Syncthing runs as a systemd service, you can use `journalctl` to inspect its logs:

```bash
# Follow logs in real-time
journalctl -u syncthing -f
# View latest logs (end of file)
journalctl -u syncthing -e
```

### Service Status
Check if the service is active and see a brief summary of recent logs:
```bash
systemctl status syncthing
```

### CLI Utilities
Syncthing also provides a CLI for basic information:
```bash
# Check installed version
syncthing version

# Manually trigger a rescan of all folders
syncthing cli operations rescan
```
