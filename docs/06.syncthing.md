...
| `extraFlags` | `[ ]` | Extra CLI arguments for the binary. |
...
In Syncthing v2.x (Hafnium Hornet), the flag `--no-default-folder` has been removed. NixOS modules (especially in 25.11) may attempt to pass this flag to enforce a purely declarative setup. If you encounter a startup failure with an "unknown flag" error, ensure that `extraFlags` is empty or does not contain this flag. 
