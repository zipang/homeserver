...
1.  **SSH Keys**: Place your authorized public keys in `/etc/nixos/ssh/authorized_keys`. The configuration points to this file for both `zipang` and `master` users.
2.  **Passwords**: User passwords are not stored in Git. After the first deployment, set them manually:
    ```bash
    sudo passwd master
    sudo passwd zipang
    ```
3.  **Disk Labels**: Your storage drive must be labeled `MEDIAS` and your swap partition `SWAP`.
4.  **Hardware Config**: The file `hosts/SKYLAB/hardware-configuration.nix` must match your server's hardware (auto-generated by NixOS).
